...
`herdr-boss lanes` prints one line per quota provider: open, ahead of pace, or near exhaustion. For a provider ahead of pace it also prints when the provider is back on pace if it is not used. The bulletin has the same data in its "Provider lanes" section. `worker start` refuses a provider that is ahead of pace or near exhaustion unless `--force` is set. When every metered provider is ahead of pace, it allows the least-over provider with a notice. A quota window whose reset time has passed shows "reset, not yet measured" until the next reading, and pacing ignores its old percentage.
...
A browser can have several tabs, and agents can work in different tabs at the same time. The tab list reloads at least every 10 seconds. A tab label that starts with **Agent** marks a tab where an agent's browser driver has a DevTools session. You can take screenshots of any tab, because a screenshot does not change the page. Navigation, history, and input on an agent tab need one confirmation per tab. The confirmation lasts until you reload the dashboard. Use **New tab** to open a blank tab of your own in a background window; agent tabs stay unchanged. A tab label that starts with **Hidden** marks a page that is not visible; some web apps do not draw in a hidden page. A screenshot fails after 15 seconds when the page does not produce a frame. The page can be loading, busy, or showing a dialog.
...
| `POST /api/browser-sessions/input` | Send an opt-in click, text, or key command to a managed page. On an agent tab, navigation, history, and input return 409 unless the body has `confirmAttached: true`. |
| `POST /api/browser-sessions/new-tab` | Open a blank background tab and return its `id`. |
