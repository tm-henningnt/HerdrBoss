...
A browser can have several tabs, and agents can work in different tabs at the same time. The tab list reloads at least every 10 seconds. A tab label that starts with **Agent** marks a tab where an agent's browser driver has a DevTools session. You can take screenshots of any tab, because a screenshot does not change the page. Navigation, history, and input on an agent tab need one confirmation per tab. The confirmation lasts until you reload the dashboard. Use **New tab** to open a blank background tab of your own; agent tabs stay unchanged. A screenshot fails after 15 seconds when the page does not produce a frame. The page can be loading, busy, or showing a dialog.
